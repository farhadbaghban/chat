...
class UserProfileManager(BaseUserManager):
    def create_user(
        self, username, first_name, last_name, password=None, email=None, is_active=True
    ):
        if not username:
    ...
    def create_superuser(
        self, username, first_name, last_name, password, is_superuser=True
    ):
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def unread_count(self):
        return self.sent_messages.filter(is_read=False).count()
